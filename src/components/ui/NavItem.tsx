...
export function NavItem({ icon, label, active, onClick }: NavItemProps) {
  return (
    <button
      ...
      style={active
        ? { background: "var(--border-primary)", color: "var(--text-primary)", border: "1px solid var(--border-secondary)" }
    ...
    >
      ...
      <span className="absolute top-full mt-2 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-[var(--border-primary)] text-[var(--text-secondary)] text-xs px-2 py-1 rounded whitespace-nowrap pointer-events-none z-50">
        {label}
      </span>
    </button>
  );
